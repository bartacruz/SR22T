...
#############
#testing
#props.globals.getNode("/environment_jpc/dewpoint-degc",1).setDoubleValue(-2);
#props.globals.getNode("/environment_jpc/temperature-degc",1).setDoubleValue(-10);
#props.globals.getNode("/environment_jpc/airspeed-kt",1).setDoubleValue(160);
#props.globals.getNode("/environment_jpc/effective-visibility-m",1).setDoubleValue(500);
#var dewpointN     = props.globals.getNode( "/environment_jpc/dewpoint-degc" );
#var temperatureN  = props.globals.getNode( "/environment_jpc/temperature-degc" );
#var speedN        = props.globals.getNode( "/environment_jpc/airspeed-kt" );
#var icingRootN    = props.globals.getNode( "/environment_jpc/icing", 1 );
#var visibilityN   = props.globals.getNode( "/environment_jpc/effective-visibility-m" );
################
...
IceSensitiveElement.new = func {
  ...
  if( n != nil ) {
    n = n.getValue();
    if( n != nil ) {
      obj.controlN = props.globals.initNode( n, 0, "DOUBLE" );
    }
  }
...
IceSensitiveElement.update = func {
	...
	if( me.controlN != nil ){
		var deicing_system = me.controlN.getValue();
		if(me.nameN.getValue()=="Wings"){
			deicing_system = deicing_system + 1;#for the wings, the deicing system is -1 to 2 ... 0 is low deicing
		}
		
		if(me.nameN.getValue()=="Windshield" and getprop("/controls/switches/defrost-lever")>0){##defrost lever
			deicing_system = 2;
		}
		if( deicing_system>0) {
			var decing_level = deicing_system * 0.005;
			if( me.iceAmountN.getValue() != 0.0 ) {
				me.iceAmountN.setDoubleValue(me.iceAmountN.getValue() - decing_level);
				if(me.iceAmountN.getValue()<0){
	...
	##weight of ice
	if(me.nameN.getValue()=="Wings"){
		## weight max = airplane mass / 10 * amount of icing
		var weight = me.iceAmountN.getValue() * 1761 / 10;
		setprop("/sim/icing/ice-weight/wings",weight);
	}
	if(me.nameN.getValue()=="Fuselage"){
		var weight = me.iceAmountN.getValue()* 1761 / 10;
		setprop("/sim/icing/ice-weight/fuselage",weight);
	}
};
